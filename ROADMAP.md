...
- **Handoff / next experiment:** Execute one concrete scenario on a machine that owns the immutable target tree, exact unpatched BB-BL1 shadPS4 binary and capture/input tooling using the bounded runner and semantic oracle from `docs/experiments/target-execution-feasibility.md`. The current handoff rejects both explicit `--emulator-config` and non-empty `--patch-commit`: the pinned CLI does not bind an arbitrary config path to consumed settings, and commit labels alone do not prove patch existence/order/application or contribution to the executable. Patched runs therefore require a future independently verified checkout/build-provenance input. Analyze only the resulting safe ZIP; keep opaque captures externalized.
- **Acceptance / artifacts:** `docs/experiments/target-execution-feasibility.md` records the route, required inputs, isolation rules, oracle choices, unsupported claims, and operator procedure. `schemas/target-run.schema.json` v3, `tools/run_target_experiment.py`, the v3 synthetic scenario, and tests define and validate the one-shot command contract, versioned run record, exception-safe bounded termination, fail-closed unpatched-source/config provenance, safe target/scenario projections, enum-only embedded strings, and independent packaged-payload identities.
- **Validation:** Exact substantive validation commit `347cf7b85fd0a79d5bdd2201e2227c42e4702462` passed Target run contract run `31912451101`, Agentic repository contract run `31912451081` (pinned ARK 0.1.9), Bloodborne target manifest run `31912451130`, and Host environment manifest run `31912451082` on Ubuntu, macOS, and Windows. Synthetic/contract coverage establishes strict finite JSON parsing; exact BB-BL2 target-tree and direct-emulator argv binding; fail-closed rejection of unattested emulator config files and all non-empty patch lists until checkout/build provenance can be verified independently; schema-valid safe target/scenario projections with separate raw and packaged digests; repository-registered enum-only embedded strings; stale-output rejection; Windows Job Object and Linux subreaper/process-group bounding including detached-session descendants; exception-safe teardown under interrupted waits/control-path failures while preserving the original exception; source and packaged-redacted artifact digests; and bounded teardown accounting. macOS validates portable contracts/collector behavior but target execution itself fails closed outside Windows/Linux. This evidence does not establish Bloodborne/shadPS4 runtime behavior; the first target execution remains gated.
- **Scope:** Medium
