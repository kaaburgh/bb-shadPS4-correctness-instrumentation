...
- **Validation:** Exact-head CI for these slices exercises unit/synthetic identity, reconstruction, and graphics-key-surface checks only. The key-surface checker now establishes 15/21 exact canonicalizations and explicitly reports exact identity not ready with 6 fields still missing; these checks do not establish target runtime coverage, exact pipeline identity, GPU timestamp semantics, or target overhead.
- **Scope:** Medium
