...
- **Result / evidence:** Static inspection at exact BB-BL1 baseline identified the resource/sync/readback seams from the prior slice and now also establishes the direct guest-CPU page-fault observer chain: on the non-userfaultfd access-violation path, protected GPU-mapped writes dispatch to `Rasterizer::InvalidateMemory` and reads to `Rasterizer::ReadMemory`; under `ENABLE_USERFAULTFD`, static source establishes write-protect fault observation but not an equivalent direct-read fault path. Synthetic `bb-trace-events/v1` reconstruction validates create→access→sync→destroy ordering and preserves complete correlation metadata. Evidence is `static` + `synthetic`; no Bloodborne runtime execution, producer implementation, or observer completeness is established, and missing direct-read events remain `unknown` when exact host/build fault-path coverage is unproven.
- **Next experiment / information gain:** Design and implement a bounded diagnostic producer at the page-fault/rasterizer boundary that emits `guest_cpu` access events with exact host/build/fault-mechanism provenance, then independently exercise every claimed observer path with a known-access control or structural seam-coverage oracle. Preserve observed/unknown/unobserved/ambiguous coverage; explicitly cover the userfaultfd read limitation or establish a separate read observer before admitting any negative `GPU-only` claim.
- **Acceptance / artifacts:** `tools/resource_sync_trace.py`, `tests/test_resource_sync_trace.py`, `docs/instrumentation/examples/resource-sync.synthetic.json`, `.github/workflows/resource-sync-trace.yml`, and `docs/instrumentation/resources-sync.md` provide the current bounded reconstruction/static-seam slice. Completion still requires evidence-backed bounded diagnostic production of direct guest CPU read/write observations for tracked GPU-backed ranges, correlation with resource lifetime/GPU activity, exact observer provenance, and independent coverage validation for every path supporting a negative claim. `GPU-only` remains inadmissible where direct-read coverage is missing or unknown; diagnostic mode remains disabled by default.
- **Validation:** Exact-head CI for this static seam/reconstruction work exercises repository contracts and the synthetic resource-sync checks only. These checks do not establish target runtime coverage, producer behavior, userfaultfd read coverage, or observer completeness.
- **Scope:** Medium
