...
- **Handoff / next experiment:** Establish a reproducible clean termination mechanism for one bounded supported run; preserve this timeout as negative evidence and resolve material host/config provenance unknowns. Use `tools/shadps4_build_manifest.py build` on a clean exact source checkout (including recursive submodules), retain its private manifest, and pass it with `--source-checkout` to `tools/run_target_experiment.py run`. A patched run must repeat the manifest's full ordered `--patch-commit` list; Git parent links, effective HEAD/tree and staged binary digest/size are verified, not inferred from declarations. Use a verified disposable CUSA03173 base 01.00 copy, a new portable profile and a bounded `process-exit` scenario, excluding adjacent update 01.09, saves, patches and old AppImage/config. Analyze only the safe ZIP. Explicit emulator config, non-synthetic file oracles and declared artifacts remain gated.
- **Acceptance / artifacts:** `docs/baseline/source-builds.md`, `schemas/shadps4-build.schema.json`, the producer/verifier and `docs/experiments/target-execution-feasibility.md` describe the source-first procedure. `bb-target-run/v3` now carries a safe build-provenance projection and exact private-manifest digest. `Completed and verified` requires a successful bounded target-machine run record through the supported entrypoint with required source/target/host provenance and termination. Menu/gameplay semantic checkpoints belong to later items and are not ENV1 acceptance criteria.
- **Validation:** `bb-target-runner/1.12.0` preserves immutable input snapshots, private executable staging and staged-byte hashing, bounded contained execution, target pre/post verification, safe packaging and the fail-closed internal-engine CLI boundary. Real Git/CMake/Ninja synthetic controls run both baseline and two-commit patched builds end-to-end; negatives reject wrong commit/tree, dirty/hidden source state, missing or misordered patch identity, wrong effective HEAD/tree, submodule drift and wrong binary digest/size. Exact local target-run outcome and completion decision are recorded in the linked experiment; capability tests alone do not complete this item.
- **Scope:** Medium
