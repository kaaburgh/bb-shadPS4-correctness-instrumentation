...
- **Result / evidence:** Static inspection at exact BB-BL1 baseline identified `PipelineCache::RefreshGraphicsKey`/`GetGraphicsPipeline` and `Scheduler::BeginRendering`/`SubmitExecution` as candidate graphics/pipeline/render-lifecycle seams. The earlier synthetic `bb-trace-events/v1` reconstruction correlates capture-local graphics events with pipeline IDs and separate CPU/GPU timing spans while preserving resource/queue/pipeline/span correlation. Deterministic source-bound shader identities, a bounded `pipeline_family_identity`, and render identities with explicit color/depth/stencil attachment roles are established synthetically from pinned source semantics. The complete pinned 21-field top-level/flattened `GraphicsPipelineKey` equality surface is inventoried in `docs/instrumentation/graphics-pipeline-key-surface.json`; exact canonicalization now covers 20/21 fields: the prior 19 canonicalized fields plus `color_buffers` as eight semantic `PsColorBuffer` records preserving the assigned 6-bit data-format, 4-bit number-format, 3-bit conversion, 4-bit export-format, and four-component `u8` swizzle domains. The remaining 1/21 field—`blend_controls`—is missing and `pipeline_identity_ready` remains false. Evidence is `static` + `synthetic`; no Bloodborne runtime execution, exact cross-run pipeline identity, runtime creation/cache producer, GPU timestamp semantics, target coverage, or measured instrumentation overhead is established.
- **Next experiment / information gain:** Independently canonicalize the remaining equality-participating pinned `GraphicsPipelineKey` field—`blend_controls`—before defining an exact cross-run pipeline identity; keep the current family digest as a coarser descriptor. After all 21 canonicalizations are independently reviewed, add bounded creation-versus-cache observations at `GetGraphicsPipeline`. Separately establish a bounded runtime timing producer with independently justified CPU/GPU timing semantics; do not treat optional Tracy GPU scopes as the BB timing source without independent evidence.
...
- **Validation:** Exact-head CI for these slices exercises unit/synthetic identity, reconstruction, and graphics-key-surface checks only. The key-surface checker now establishes 20/21 exact canonicalizations and explicitly reports exact identity not ready with 1 field still missing; these checks do not establish target runtime coverage, exact pipeline identity, GPU timestamp semantics, or target overhead.
- **Scope:** Medium
