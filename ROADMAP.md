...
- **Validation:** Target-run schema/scenario v2 and 44 local tests establish strict input validation, explicit per-artifact allowlists with fail-closed unknown-field handling, stale-output rejection, process-group/Job Object bounding, artifact redaction, and ZIP contents. They do not establish Bloodborne/shadPS4 runtime behavior; the first target execution remains gated.
- **Scope:** Medium
