...
- **Local feasibility audit (2026-09-08):** [Direct host/target inspection and bounded diagnostic probes](./docs/experiments/local-environment-audit-2026-09-08.md) establish that this agent can access the target-owning Linux machine and X11 display `:1`; the historical synthetic-only checkout observation no longer describes all available execution inputs. The installed UltraPersona AppImage differs from the admitted BB-BL1 executable; GitHub reports the pinned Linux artifact expired and its download returns HTTP 410. Base `01.00` and adjacent update `01.09` must be explicitly resolved. Separate clean-profile probes are diagnostic observations, not supported BB-ENV1 run records or gameplay validation. Status and downstream gates remain unchanged.
- **Handoff / next experiment:** First recover an archived copy of the exact pinned Linux artifact and verify its committed digest, or resolve build-provenance admission in a separate bounded change (existing PR #127 is not part of this baseline). Prepare the complete resolved target/config identity and isolated target view on this already-accessible host; no new graphics-session handoff is needed. Then execute one bounded `process-exit` scenario on a target-owning machine through `tools/run_target_experiment.py` only and retain the resulting safe ZIP/run record as the validation evidence for this item. Non-synthetic execution is limited to the exact independently observed unpatched BB-BL1 CI artifact for the host; the runner copies the selected regular non-link executable into private per-run staging, verifies staged digest/size against the pinned artifact and caller-supplied digest, rewrites the snapshotted `argv[0]` to that staged path, then delegates to the normal bounded compatibility-engine executor. Direct execution of `tools/run_target_experiment_v3.py` fails closed. The handoff rejects explicit `--emulator-config`, non-empty `--patch-commit`, non-synthetic file oracles, and non-synthetic declared artifacts until those provenance relationships can be independently attested. Analyze only the resulting safe ZIP.
