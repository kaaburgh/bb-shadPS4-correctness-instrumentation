...
- **Validation:** Current PR contract coverage includes strict finite JSON parsing; exact BB-BL2 target-tree and direct-emulator argv binding; fail-closed unpatched-source/config provenance; exact official CI-produced executable identity; single-snapshot target/scenario/command inputs; private executable staging with pre-delegation digest/size verification and a Linux non-synthetic-classified end-to-end stand-in regression; fail-closed direct compatibility-engine invocation; non-synthetic oracle/artifact producer gating; safe target/scenario/DLC projections and packaged digests; stale-output rejection; process-tree containment and exception-safe teardown. `bb-target-runner/1.11.0` distinguishes this mechanism from the retired executable-lease design. Exact-head GitHub Actions must remain green before this PR can be treated as verified. These synthetic/contract checks do not establish Bloodborne/shadPS4 runtime behavior; the first target execution remains gated.
- **Scope:** Medium
...
- **Result / evidence:** Static inspection at exact BB-BL1 baseline identifies the resource/sync/readback seams and establishes the direct guest-CPU page-fault observer chain: on the non-userfaultfd access-violation path, protected GPU-mapped writes dispatch to `Rasterizer::InvalidateMemory` and reads to `Rasterizer::ReadMemory`; under `ENABLE_USERFAULTFD`, static source establishes write-protect fault observation but not an equivalent direct-read fault path. The bounded producer-design slice places observation after the rasterizer GPU-mapped acceptance boundary, requires deterministic live-resource correlation, and preserves host/build/fault-mechanism coverage as an explicit evidence concern. It also establishes a trace-contract compatibility gap: `bb-trace-events/v1` has whole-stream producer/baseline provenance but no observer/fault-mechanism capability provenance sufficient to make missing direct-access events negative evidence. v1 therefore now fails closed for runtime `guest_cpu` events with `coverage=unobserved`; synthetic `unobserved` remains available only for contract/consumer tests. Evidence is `static` + `synthetic`; no Bloodborne runtime execution, producer implementation, observer completeness, or negative `GPU-only` claim is established.
- **Next experiment / information gain:** Version a bounded observer-provenance boundary that identifies the exact fault mechanism and independently established read/write coverage capabilities, with fail-closed consumer compatibility. Only after that boundary exists, implement the diagnostic producer at the accepted page-fault/rasterizer seam and independently exercise every claimed observer path with a known-access control or structural seam-coverage oracle. Keep userfaultfd direct-read coverage `unknown` unless a separate read observer is established; do not admit negative `GPU-only` evidence from missing events alone.
- **Acceptance / artifacts:** `tools/resource_sync_trace.py`, `tests/test_resource_sync_trace.py`, `docs/instrumentation/examples/resource-sync.synthetic.json`, `.github/workflows/resource-sync-trace.yml`, `docs/instrumentation/resources-sync.md`, and `docs/instrumentation/guest-cpu-observer-producer.md` provide the current reconstruction/static-seam/producer-boundary slices. `tools/trace_event_model.py`, `tests/test_trace_event_model.py`, and `docs/instrumentation/schema.md` fail closed on runtime `guest_cpu/unobserved` until observer provenance is versioned. Completion still requires a versioned observer/capability provenance contract, evidence-backed bounded diagnostic production of direct guest CPU read/write observations for tracked GPU-backed ranges, correlation with resource lifetime/GPU activity, and independent coverage validation for every path supporting a negative claim. Diagnostic mode remains disabled by default.
- **Validation:** Exact-head CI for this slice exercises the trace-event contract plus repository/resource-sync synthetic checks only. These checks validate the v1 negative-evidence fail-close and static/synthetic contracts; they do not establish Bloodborne runtime coverage, producer behavior, userfaultfd read coverage, observer completeness, or target overhead.
- **Scope:** Medium
...
- **Result / evidence:** Static inspection at exact BB-BL1 baseline identified `PipelineCache::RefreshGraphicsKey`/`GetGraphicsPipeline` and `Scheduler::BeginRendering`/`SubmitExecution` as candidate graphics/pipeline/render-lifecycle seams. The earlier synthetic `bb-trace-events/v1` reconstruction correlates capture-local graphics events with pipeline IDs and separate CPU/GPU timing spans while preserving resource/queue/pipeline/span correlation. Deterministic source-bound shader identities, a bounded `pipeline_family_identity`, and render identities with explicit color/depth/stencil attachment roles are established synthetically from pinned source semantics. The complete pinned 21-field top-level/flattened `GraphicsPipelineKey` equality surface is inventoried in `docs/instrumentation/graphics-pipeline-key-surface.json`; exact canonicalization now covers 17/21 fields: the prior 16 canonicalized fields plus `write_masks` as eight complete 32-bit raw-pattern values. The remaining 4/21 fields are missing and `pipeline_identity_ready` remains false. Evidence is `static` + `synthetic`; no Bloodborne runtime execution, exact cross-run pipeline identity, runtime creation/cache producer, GPU timestamp semantics, target coverage, or measured instrumentation overhead is established.
- **Next experiment / information gain:** Independently canonicalize the remaining 4 equality-participating pinned `GraphicsPipelineKey` fields—shader hashes, Vulkan enums and nested packed state—before defining an exact cross-run pipeline identity; keep the current family digest as a coarser descriptor. After all 21 canonicalizations are independently reviewed, add bounded creation-versus-cache observations at `GetGraphicsPipeline`. Separately establish a bounded runtime timing producer with independently justified CPU/GPU timing semantics; do not treat optional Tracy GPU scopes as the BB timing source without independent evidence.
- **Acceptance / artifacts:** `tools/graphics_timing_trace.py`, `tests/test_graphics_timing_trace.py`, `docs/instrumentation/examples/graphics-timing.synthetic.json`, `.github/workflows/graphics-timing-trace.yml`, and `docs/instrumentation/graphics-timing.md` provide the capture-local reconstruction slice. `tools/graphics_identity_model.py`, `tests/test_graphics_identity_model.py`, `docs/instrumentation/examples/graphics-identity.synthetic.json`, and `.github/workflows/graphics-identity-model.yml` provide the safe shader/render/pipeline-family slice. `docs/instrumentation/graphics-pipeline-key-surface.json`, `tools/graphics_pipeline_key_surface.py`, `tests/test_graphics_pipeline_key_surface.py`, and `.github/workflows/graphics-pipeline-key-surface.yml` provide the exact pinned equality-surface inventory/checker and fail closed while any canonicalization remains missing. Completion still requires evidence-backed exact safe cross-run pipeline identity, creation/cache observations, a runtime timing producer with established CPU/GPU semantics, target coverage and measured tracing overhead; no proprietary shader payload may be committed.
- **Validation:** Exact-head CI for these slices exercises unit/synthetic identity, reconstruction, and graphics-key-surface checks only. The key-surface checker now establishes 17/21 exact canonicalizations and explicitly reports exact identity not ready with 4 fields still missing; these checks do not establish target runtime coverage, exact pipeline identity, GPU timestamp semantics, or target overhead.
- **Scope:** Medium
