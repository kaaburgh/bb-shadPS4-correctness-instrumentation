...
- **Result / evidence:** Static inspection at exact BB-BL1 baseline identified `PipelineCache::RefreshGraphicsKey`/`GetGraphicsPipeline` and `Scheduler::BeginRendering`/`SubmitExecution` as candidate graphics/pipeline/render-lifecycle seams. The earlier synthetic `bb-trace-events/v1` reconstruction correlates capture-local graphics events with pipeline IDs and separate CPU/GPU timing spans while preserving resource/queue/pipeline/span correlation. Deterministic source-bound shader identities, a bounded `pipeline_family_identity`, and render identities with explicit color/depth/stencil attachment roles are established synthetically from pinned source semantics. The complete pinned 21-field top-level/flattened `GraphicsPipelineKey` equality surface is inventoried in `docs/instrumentation/graphics-pipeline-key-surface.json`; exact canonicalization now covers all 21/21 fields. `blend_controls` is represented as eight exact 32-bit raw guest register words so unnamed/reserved bits that participate in the pinned bytewise key representation are not discarded, and the checker reports `pipeline_identity_ready=true`. Evidence is `static` + `synthetic`; no Bloodborne runtime execution, emitted exact cross-run pipeline identity, runtime creation/cache producer, GPU timestamp semantics, target coverage, or measured instrumentation overhead is established.
- **Next experiment / information gain:** Define the exact safe cross-run pipeline identity from the independently reviewed 21-field canonical representation, while retaining the existing family digest as a coarser descriptor. Then add bounded creation-versus-cache observations at `GetGraphicsPipeline`. Separately establish a bounded runtime timing producer with independently justified CPU/GPU timing semantics; do not treat optional Tracy GPU scopes as the BB timing source without independent evidence.
- **Acceptance / artifacts:** `tools/graphics_timing_trace.py`, `tests/test_graphics_timing_trace.py`, `docs/instrumentation/examples/graphics-timing.synthetic.json`, `.github/workflows/graphics-timing-trace.yml`, and `docs/instrumentation/graphics-timing.md` provide the capture-local reconstruction slice. `tools/graphics_identity_model.py`, `tests/test_graphics_identity_model.py`, `docs/instrumentation/examples/graphics-identity.synthetic.json`, and `.github/workflows/graphics-identity-model.yml` provide the safe shader/render/pipeline-family slice. `docs/instrumentation/graphics-pipeline-key-surface.json`, `tools/graphics_pipeline_key_surface.py`, `tests/test_graphics_pipeline_key_surface.py`, and `.github/workflows/graphics-pipeline-key-surface.yml` now provide a complete exact pinned equality-surface canonicalization contract. Completion still requires an evidence-backed emitted exact safe cross-run pipeline identity, creation/cache observations, a runtime timing producer with established CPU/GPU semantics, target coverage and measured tracing overhead; no proprietary shader payload may be committed.
- **Validation:** Exact-head CI for these slices exercises unit/synthetic identity, reconstruction, and graphics-key-surface checks only. The key-surface checker now establishes 21/21 exact canonicalizations and reports `pipeline_identity_ready=true`; these checks establish readiness of the static canonical key surface, not an emitted exact pipeline identity, target runtime coverage, GPU timestamp semantics, or target overhead.
- **Scope:** Medium
