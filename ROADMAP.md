...
- Dirty diagnostic instrumentation и local experimental patches разрешены для проверки hypotheses; promotion correctness implementation требует established semantic seam/compatibility boundary.
...
- **Exploration is the default.** Reuse persistent/dirty source, incremental builds, existing binaries/caches/artifacts and one verified disposable working target copy. Original proprietary inputs remain immutable. No clean manifest/worktree or completed **BB-ENV1** is required to explore.
- **BB-ENV1** is a capability/verification item. Explicit ENV1 dependencies below gate promotion-grade acceptance only, never exploratory launch, candidate scenarios, instrumentation, hypotheses or local patches. Technical evidence dependencies remain in force. Failure to satisfy promotion-grade provenance prevents promotion of a claim, not the experiment.
- Supported runs remain bounded and package safe records; exploratory observations are explicitly unverified. Strict source-build/target/config provenance is opt-in for promotion; proprietary executables/assets/private dumps не коммитить.
- Promoted runtime claims фиксируют shadPS4 repo+exact commit+patches, Bloodborne build/content/update/config, host OS/CPU/GPU/driver/backend/config, scenario и tool version.
...
Exploratory steps may proceed with available target/build state regardless of ENV1's
`Implemented, validation incomplete` status. BB-BL4 candidate exercise is ready;
launch/termination diagnostics and instrumentation hypothesis tests need no ENV1
permission. `Depends on` records technical inputs or explicitly identified
promotion prerequisites, not admission to every intermediate experiment. Item
completion/confirmed evidence still needs its acceptance criteria and dependencies;
unverified observations do not close those gates. Do not use this distinction to
bypass BB-COR7/BB-INS4 optimization/corpus gates or fabricate missing technical
inputs. `GATED` describes target-machine/verification needs, not a blanket ban on
exploration. See [the supported workflow](docs/experiments/target-execution-feasibility.md).

Dependency review: removed **BB-ENV1 → BB-BL4** only. A first successful verified
ENV1 termination record supplies no technical input for trying candidate actions
or identifying possible gameplay scenarios. BB-BL4 still depends on BL1/BL2/BL3
baseline definitions and requires exact reproducible identity plus independent
checkpoint evidence before selection. Kept ENV1 on BL6 (reproducible distributions),
COR2–COR5 (confirmed reproduction/classification), FIX2/FIX4/FIX6 (validated fixes),
INS4 (confirmed coverage/overhead), SHD2/RES2/PERF2 (reproducible datasets): those
acceptance claims require promoted target evidence. All other scientific/technical
edges are unchanged.
...
- **Result / evidence:** 2026-09-09 static release inspection adopted v0.18.0, pinning the emulator core at `shadps4-emu/shadPS4@e3ce810f3a653f43ac64ebab63023de281a4103a`; source changes use exact-base fork/topic commits while this repository records immutable source/build provenance. See `docs/experiments/baseline-v0180-adoption.md`: static mapping/patch inputs reconciled; previous runtime/capture evidence cannot be transferred across the changed guest/shader/stencil semantics. No existing completed target capture is available to reopen. ENV1 source-build verification was implemented in #130; its failed run remains negative evidence, not an exploration blocker.
...
- **Evidence / decision:** Schema v1 разделяет descriptive labels от field-level evidenced exact identity, сохраняет explicit partial identity и задаёт fail-closed three-way comparison (`same` / `different` / `indeterminate`) с учётом unknown pointers. Synthetic fixture остаётся format-only. Runtime/static evidence on the target-owning node now records the active EU `CUSA03173` view as base `01.00` plus the shadPS4-native sibling `CUSA03173-UPDATE` overlay at effective `01.09`; the safe identity is checked in as `docs/baseline/bloodborne-target-1.09.json`. The reusable disposable route binds both `app` and `app-UPDATE` and its one-time resolved-tree identity before exploration. This identifies the target baseline and launch version, not gameplay correctness.
- **Acceptance / artifacts:** [`docs/baseline/bloodborne.md`](./docs/baseline/bloodborne.md) + [JSON Schema](./schemas/bloodborne-target-manifest.schema.json) + [active 1.09 manifest](./docs/baseline/bloodborne-target-1.09.json) + [synthetic example](./docs/baseline/examples/bloodborne-target-manifest.synthetic.json) + [validator/comparator](./tools/bloodborne_target_manifest.py) + [tests](./tests/test_bloodborne_target_manifest.py) cover build/content/update/config state, hashing/comparison semantics and licensing/privacy boundary without proprietary payload.
- **Validation:** Draft 2020-12 schema, executable semantic checks и positive/negative synthetic cases validate format/internal consistency; the active manifest validates against the same schema and its resolved tree was independently hashed from the existing base plus update overlay. The disposable-copy v2 receipt contract requires the `app-UPDATE` sibling for an installed update and rejects a base-only route. The exploratory launch independently reported `App Version: 01.09`; no semantic gameplay/correctness claim is implied.
- **Scope:** Small
...
- **Question:** Может ли required Bloodborne target execution быть воспроизводимо выполнен в доступной infrastructure; какой concrete target-machine route необходим?
- **Evidence / result:** The original synthetic-only Git checkout observation does not describe all local host resources. PR #128 reported an available Ubuntu target/display and HTTP 410 for the historical Linux CI artifact. PR #127 proposed patched admission but did not verify Git ancestry and retained the unpatched CI dependency; neither stale branch was layered into this work. The active source baseline is v0.18.0. Source-first admission now accepts locally built exact baseline and fully identified linear patched states through a durable project-produced build manifest. The actual local RelWithDebInfo/Clang 19.1.1 ELF passed supported admission on a verified disposable CUSA03173 base 01.00 copy. Its one 30-second attempt ended `timed_out` (exit -15, 34.064 seconds with teardown), oracle `unknown`, packaging `complete`, target pre/post verified. Safe records and separate static/synthetic evidence are in `docs/experiments/env1-source-first-2026-09-09.md`; this failure record does not complete ENV1.
- The reusable disposable route now contains independent `app` and native sibling `app-UPDATE` trees bound by a v2 receipt and the active resolved-tree identity. A bounded runner launch through that root, followed by a same-root runtime probe, reported effective `App Version: 01.09`; this is launch/liveness evidence only and does not complete ENV1.
- **Handoff / next experiment:** Explore a bounded clean-termination mechanism using the existing durable binary/build and the reusable disposable CUSA03173 1.09 copy with its `app-UPDATE` sibling; preserve the base-only 01.00 timeout as historical negative evidence. Dirty diagnostics and incremental rebuilds need no manifest. When ready to verify ENV1 completion, use the strict source-build manifest route, full target pre/post verification, controlled profile/config and resolve material host/config unknowns. Reuse a still-valid strict manifest/binary where possible; use a fresh strict build only if promotion requires changed source/build state. Analyze safe records; exploratory file/capture observations do not confer semantic verification.
- **Acceptance / artifacts:** `docs/baseline/source-builds.md`, `schemas/shadps4-build.schema.json`, the producer/verifier and `docs/experiments/target-execution-feasibility.md` describe the source-first procedure. `bb-target-run/v4` distinguishes exploration from verification candidates and carries a safe build-provenance projection and exact private-manifest digest. `Completed and verified` requires a successful bounded target-machine run record through the supported entrypoint with required source/target/host provenance and termination. Menu/gameplay semantic checkpoints belong to later items and are not ENV1 acceptance criteria.
- **Validation:** #130 `bb-target-runner/1.12.0` established the strict path; `1.13.1` retains it as opt-in and adds schema-v4 unverified exploration. PR #131 review hardening binds target evidence classes (including synthetic leaves), keeps material host/config completeness in semantic review instead of a zero-unknown counter gate, and requires a cheap disposable-copy receipt for exploration. Strict verification preserves immutable input snapshots, private executable staging and staged-byte hashing, bounded contained execution, target pre/post verification, safe packaging and the fail-closed internal-engine CLI boundary. Real Git/CMake/Ninja synthetic controls run both baseline and two-commit patched builds end-to-end; negatives reject wrong commit/tree, dirty/hidden source state, missing or misordered patch identity, wrong effective HEAD/tree, submodule drift and wrong binary digest/size. Exact local target-run outcome and completion decision are recorded in the linked experiment; capability tests alone do not complete this item.
- **Scope:** Medium
...
- **Result / evidence:** `docs/scenarios/README.md` now defines the durable scenario-entry template, baseline/evidence fields, bounded actions/end conditions, oracle-strength boundary, and candidate→selected gate. Static inspection of the existing BB-ENV1 route established that strict verification runs accept only the `process-exit` oracle and no declared artifacts: this can bind exact provenance and bounded termination, but it cannot independently attest a title-visible gameplay/correctness/performance checkpoint. Evidence is `static`; the separate direct 01.09 exploratory launch is recorded, but it did not select a scenario or provide semantic/runner-bound correctness evidence. That process-exit/artifact restriction now applies only to strict verification. Candidate exercise is permitted now on the exploratory path; no candidate has been selected by this workflow change.
- **Next experiment / information gain:** Exercise bounded candidates now on a target-owning machine through the exploratory route and retain only safe evidence. Keep operator checkpoint observations as `reported`; promote a candidate to `selected` only when the expected observable has an independent evidence path appropriate to the claim. If the current runner cannot produce that semantic evidence, add a producer-bound oracle/artifact path in its own bounded work before relying on it for selection.
...
- **Next experiment / information gain:** Offline clustering/counterexamples; a required target discriminating matrix may be explored immediately with available technical inputs; add an ENV1 dependency only if its acceptance requires promotion-grade verified evidence.
