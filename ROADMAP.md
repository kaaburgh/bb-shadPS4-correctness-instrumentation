...
- **Result / evidence:** Static inspection at exact BB-BL1 baseline identified `PipelineCache::RefreshGraphicsKey`/`GetGraphicsPipeline` and `Scheduler::BeginRendering`/`SubmitExecution` as candidate graphics/pipeline/render-lifecycle seams. The earlier synthetic `bb-trace-events/v1` reconstruction correlates capture-local graphics events with pipeline IDs and separate CPU/GPU timing spans while preserving resource/queue/pipeline/span correlation. This slice additionally derives deterministic source-bound shader identities, a bounded `pipeline_family_identity`, and render identities with explicit color/depth/stencil attachment roles from pinned `GraphicsPipelineKey`/render-state semantics. The pipeline-family projection is intentionally not an exact pipeline ID because pinned upstream equality contains materially distinguishing state outside the bounded allowlist. Evidence is `static` + `synthetic`; no Bloodborne runtime execution, exact cross-run pipeline identity, runtime creation/cache producer, GPU timestamp semantics, target coverage, or measured instrumentation overhead is established.
- **Next experiment / information gain:** Canonicalize every materially distinguishing pinned `GraphicsPipelineKey` field needed for an exact cross-run pipeline identity, and add bounded creation-versus-cache observations at `GetGraphicsPipeline`; keep the current family digest as a coarser descriptor rather than silently upgrading its semantics. Separately establish a bounded runtime timing producer with independently justified CPU/GPU timing semantics; do not treat optional Tracy GPU scopes as the BB timing source without independent evidence.
- **Acceptance / artifacts:** `tools/graphics_timing_trace.py`, `tests/test_graphics_timing_trace.py`, `docs/instrumentation/examples/graphics-timing.synthetic.json`, `.github/workflows/graphics-timing-trace.yml`, and `docs/instrumentation/graphics-timing.md` provide the capture-local reconstruction slice. `tools/graphics_identity_model.py`, `tests/test_graphics_identity_model.py`, `docs/instrumentation/examples/graphics-identity.synthetic.json`, and `.github/workflows/graphics-identity-model.yml` provide the current safe static+synthetic identity/family slice. Completion still requires evidence-backed exact safe cross-run pipeline identity, creation/cache observations, a runtime timing producer with established CPU/GPU semantics, target coverage and measured tracing overhead; no proprietary shader payload may be committed.
- **Validation:** Exact-head CI for these slices exercises unit/synthetic identity and reconstruction checks only. They establish deterministic contract behavior and the pipeline-family semantic boundary, not target runtime coverage, exact pipeline identity, GPU timestamp semantics, or target overhead.
- **Scope:** Medium
