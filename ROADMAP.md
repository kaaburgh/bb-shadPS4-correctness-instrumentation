...
- **Validation:** Exact validation commit `cb4085a8396f6324f1fd714fe2d74de658188bc2` passed Target run contract run `31911231316`, Agentic repository contract run `31911231348` (pinned ARK 0.1.9), Bloodborne target manifest run `31911231247`, and Host environment manifest run `31911231259` on Ubuntu, macOS, and Windows. Synthetic/contract coverage establishes strict finite JSON parsing, exact BB-BL2 target-tree and direct-emulator argv binding, transfer-safe target/scenario/artifact packaging, stale-output rejection, Windows Job Object and Linux subreaper/process-group bounding including detached-session descendants, source and packaged-redacted artifact digests, and bounded teardown accounting. macOS validates portable contracts/collector behavior but target execution itself fails closed outside Windows/Linux. This evidence does not establish Bloodborne/shadPS4 runtime behavior; the first target execution remains gated.
- **Scope:** Medium
