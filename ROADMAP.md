...
- **Handoff / next experiment:** Execute one concrete scenario on a machine that owns the immutable target tree, pinned shadPS4 binary and capture/input tooling using the bounded runner and semantic oracle from `docs/experiments/target-execution-feasibility.md`. On the pinned shadPS4 baseline an arbitrary config-file path cannot be proven to be the settings actually consumed, so the handoff rejects explicit `--emulator-config` rather than recording false config provenance. Analyze only the resulting safe ZIP; keep opaque captures externalized.
- **Acceptance / artifacts:** `docs/experiments/target-execution-feasibility.md` records the route, required inputs, isolation rules, oracle choices, unsupported claims, and operator procedure. `schemas/target-run.schema.json` v3, `tools/run_target_experiment.py`, the v3 synthetic scenario, and tests define and validate the one-shot command contract, versioned run record, bounded termination, pre-launch source-provenance validation, safe target/scenario projections, enum-only embedded strings, and independent packaged-payload identities.
- **Validation:** Exact substantive validation commit `9b3b5fc1b60a5e8cf1e98001b6e675738d024c49` passed Target run contract run `31911877272`, Agentic repository contract run `31911877269` (pinned ARK 0.1.9), Bloodborne target manifest run `31911877266`, and Host environment manifest run `31911877295` on Ubuntu, macOS, and Windows. Synthetic/contract coverage establishes strict finite JSON parsing; exact BB-BL2 target-tree and direct-emulator argv binding; fail-closed rejection of unattested emulator config files; max-64/unique patch provenance before launch; schema-valid safe target/scenario projections with separate raw and packaged digests; repository-registered enum-only embedded strings; stale-output rejection; Windows Job Object and Linux subreaper/process-group bounding including detached-session descendants; source and packaged-redacted artifact digests; and bounded teardown accounting. macOS validates portable contracts/collector behavior but target execution itself fails closed outside Windows/Linux. This evidence does not establish Bloodborne/shadPS4 runtime behavior; the first target execution remains gated.
- **Scope:** Medium
