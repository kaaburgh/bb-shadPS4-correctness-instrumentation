...
The mapping is bound to `shadps4-emu/shadPS4@28c84fb5a7b19c7fb86156a1d6bb3e7e5a6cef64` and the committed `bb-graphics-pipeline-key-surface/v12` document digest. It covers every one of the 21 canonical fields exactly once. Scalar/bitfield/enum values are mapped through explicit integer casts, arrays preserve their canonical element order, `cb_shader_mask` uses its explicit `raw` member, and `color_buffers` / `blend_controls` are mapped memberwise rather than by object bytes.
