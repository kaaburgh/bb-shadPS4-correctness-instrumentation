...
Cloud-first execution is **not required**. One roadmap item per PR is **required** for roadmap-scoped feature or investigation work; a process-only PR may omit a roadmap item when it explicitly explains why no roadmap item applies.
