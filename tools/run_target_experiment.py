...
def _require_non_synthetic_evidence_contract(
    ...
    if build_manifest is None or source_checkout is None:
        raise TargetRunError("non-synthetic execution requires --build-manifest and --source-checkout")
    try:
...
def run_experiment(
    *,
    ...
    build_document = None
    build_raw = None
    if (build_manifest_path is None) != (source_checkout is None):
        raise TargetRunError("build manifest and source checkout must be supplied together")
    ...
    elif not synthetic_control or patch_commits:
        raise TargetRunError("non-synthetic or patched execution requires --build-manifest and --source-checkout")
