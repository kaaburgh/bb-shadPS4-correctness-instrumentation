...
class ReviewRegressionTests(unittest.TestCase):
    ...
    def test_non_synthetic_binary_without_manifest_fails_closed(self):
        manifest = _manifest()
        manifest["provenance"]["evidence_classes"] = ["runtime"]
        with self.assertRaisesRegex(runner.TargetRunError, "build-manifest"):
            runner._require_non_synthetic_evidence_contract(
    ...
    def test_runner_version_identifies_supported_entrypoint(self):
        self.assertEqual(runner.RUNNER_VERSION, "1.12.0")
        self.assertEqual(runner._legacy.RUNNER_VERSION, "1.12.0")
